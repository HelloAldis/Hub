...
完成上述步骤之后，ViewController类里面就可以访问数据库了。4. 建立数据库模型，如下选择新建文件->Core Data->Data Model
...
当你把表和字段都建立好了之后。在new file，选择第2张图里面的NSManagedObject subclass。一路next下去。编译器将会自动给每张表生成对应的.h和 .m文件，这样数据模型就ok了。5. 插入数据
